(function () {
  ...
  window.BlogAgentAuth = {
    get: getAuth,
    set: setAuth,
    clear: clearAuth,
    headers: authHeaders,
    handleUnauthorized: sendToLogin,
  };

  function startWorkspace(auth) {
  ...
  (async function boot() {
    ...
    if (!getAuth()?.token || !(await tokenIsLive())) {
      sendToLogin();
      return;
    }
    startWorkspace(getAuth());
  })();
})();
